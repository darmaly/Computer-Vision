...

{
    ...
    if (argc != 3)
    {
    printf("Usage:  reg-grow [image] [threshold value]\n");
    exit(0);
    }
    ...
    for (c=3; c<COLS-3; c++)
        ...
        RegionGrow(thresh,image,labels,ROWS,COLS,r,c,0,TotalRegions,
            indices,&RegionSize);
        if (RegionSize < 100)
    ...
    unsigned char *rgb_image = (unsigned char *)calloc(3*ROWS*COLS,sizeof(unsigned char));
    int x, y;
    unsigned char R, G, B;
    fprintf(fpt,"P6 %d %d 255\n",COLS,ROWS);
    for(x = 0; x < ROWS; x++) {
        for(y = 0; y < COLS; y++){
            R = (labels[x*COLS+y]*53)%255;
            G = (labels[x*COLS+y]*97)%255;
            B = (labels[x*COLS+y]*223)%255;
            fwrite(&R,1,1,fpt);
            fwrite(&G,1,1,fpt);
            fwrite(&B,1,1,fpt);
        }
...
void RegionGrow(int thresh, unsigned char *image,	/* image data */
		...
		int *count)		/* output:  count of pixels painted */
{
    ...
    *count=0;
    ...
    while (qt != qh)
    {
    if ((*count)%thresh == 0)	/* recalculate average after each thresh pixels join */
        {
        average=total/(*count);
        // printf("new avg=%d\n",average);
        }
